...
```bash
modelscope download --model Qwen/Qwen3-Coder-Next-GGUF --include "Q4_K_M/*" --local_dir ./models
```
...
```bash
huggingface-cli download Qwen/Qwen3-Coder-Next-GGUF --include "Q4_K_M/*" --local-dir ./models
```

## 注意事项

下载完成后，确保 4 个 `.gguf` 文件直接放在 `./models/` 目录下，而非子目录中。
